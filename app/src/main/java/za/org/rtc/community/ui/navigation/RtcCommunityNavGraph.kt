        ...
        CompositionLocalProvider(LocalSharedTransitionScope provides this) {
            NavHost(
                ...
                composable(RtcRoute.SEARCH) {
                    SearchScreen(
                        viewModel = viewModel,
                        onOpenResult = { result -> navController.navigateOverlay(publicSearchResultRoute(result)) },
                    )
                }
                composable(
                ...
                composable(RtcRoute.AI) {
                    ProtectedRoute(RtcRoute.AI, session, onDenied = { navController.returnToSafeWorkspace(session.role.isStaff) }) {
                ...
                composable(RtcRoute.ADMIN_MFA) {
                    ProtectedRoute(RtcRoute.ADMIN_MFA, session, onDenied = { navController.returnToSafeWorkspace(session.role.isStaff) }) {
                        AdministratorMfaVerificationScreen(
                            viewModel = viewModel,
                            onReturnToWorkQueue = { navController.popBackStack() },
                        )
                    }
                }
                ...
                composable(RtcRoute.SYSTEM_HEALTH) {
                    ProtectedRoute(RtcRoute.SYSTEM_HEALTH, session, onDenied = { navController.returnToSafeWorkspace(session.role.isStaff) }) {
                        SystemHealthScreen(viewModel)
                    }
                }
                composable(RtcRoute.ADMIN_ACTIVITY) {
                    ProtectedRoute(RtcRoute.ADMIN_ACTIVITY, session, onDenied = { navController.returnToSafeWorkspace(session.role.isStaff) }) {
                        AdminActivityScreen(viewModel)
                    }
                }
                ...
                composable(RtcRoute.MY_WORK) {
                    ProtectedRoute(RtcRoute.MY_WORK, session, onDenied = { navController.returnToSafeWorkspace(session.role.isStaff) }) {
                        MyWorkProfileScreen(
                            viewModel = viewModel,
                            onOpenAccount = { navController.navigateOverlay(RtcRoute.ACCOUNT) },
                        )
                    }
                }
            }
