...
internal fun CommunityScreen(
    ...
    val displayPosts = if (tab == "Trending") feedState.items.sortedByDescending { it.trendingScore } else feedState.items
            ...
            item {
                Row(horizontalArrangement = Arrangement.spacedBy(RtcSpacing.compact)) {
                    AssistChip(
                        onClick = { tab = "Latest" },
                        label = { Text("Latest") },
                        colors = if (tab == "Latest") AssistChipDefaults.assistChipColors(containerColor = MaterialTheme.colorScheme.primaryContainer) else AssistChipDefaults.assistChipColors(),
                    )
                    AssistChip(
                        onClick = { tab = "Trending" },
                        label = { Text("Trending") },
                        colors = if (tab == "Trending") AssistChipDefaults.assistChipColors(containerColor = MaterialTheme.colorScheme.primaryContainer) else AssistChipDefaults.assistChipColors(),
                    )
                }
            }
            item { Text("Saved items, followed topics, and personal Community history are not available in this version.", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant) }
