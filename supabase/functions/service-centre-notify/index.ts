...
const INTERNAL_SECRET = Deno.env.get("SERVICE_CENTRE_INTERNAL_NOTIFY_SECRET") ?? "";
...
function safeEqual(a: string, b: string) { if (!a || a.length !== b.length) return false; let value = 0; for (let i = 0; i < a.length; i++) value |= a.charCodeAt(i) ^ b.charCodeAt(i); return value === 0; }
...
async function internalConfirmedContext(admin: ReturnType<typeof createClient>, bookingId: string) {
  const { data, error } = await admin.from("service_centre_bookings")
    .select("id,provider_user_id,category_name_snapshot")
    .eq("id", bookingId).maybeSingle();
  if (error || !data) throw new Error("BOOKING_NOT_FOUND");
  return {
    recipientUserId: data.provider_user_id,
    title: "Booking confirmed",
    body: `The ${data.category_name_snapshot} booking has been secured by the customer.`,
    payload: { notification_type: "SERVICE_BOOKING_CONFIRMED", booking_id: bookingId, route: `rtc://service-centre/booking/${bookingId}`, message_id: "" },
  };
}
...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    const internal = safeEqual(request.headers.get("x-service-centre-internal-secret") ?? "", INTERNAL_SECRET);
    let context: any;
    if (internal) {
      if (eventType !== "SERVICE_BOOKING_CONFIRMED") return json(403, { error: "Internal notifications are limited to payment confirmation." });
      context = await internalConfirmedContext(admin, bookingId);
    } else {
    ...
    const result = await deliver(admin, context);
